...
export async function createUserAdmin(req: Request, res: Response) {
  try {
    const { username, email, phoneNumber, gender, isActive, isAdmin, password } = req.body;
...
export async function getAllUsers(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({ message: `Found users`, data: users.map(user => user) });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Unknown error when getting all users!", data: [] });
  }
}
...
export async function updateUserPrivilege(req : Request, res : Response) {
  try {
    ...
    if (isAdmin !== undefined) {  // isAdmin can have boolean value true or false
      ...
      res.status(200).json({ message: `Updated privilege for user ${userId}. ${userId} is ${resultString}.` } );
    } else {
